    ...
    instance.interceptors.response.use(
      ...
      async function(error) {
        if (error.response.status === 401 && connectCnt < 5) {
          ...
          if (paypalAccessToken === 'fail') {
            SocketServer.getInstance().sendMsg2Client(req.user.accessToken, 'payment', {
              paymentStatus: 'Not',
              linceseKey: '',
              msg: 'Failed to get access token. Please contact to Perfect Forex support.'
            });
          } else {
            connectCnt++;
        ...
        SocketServer.getInstance().sendMsg2Client(req.user.accessToken, 'payment', {
          paymentStatus: 'Not',
          linceseKey: '',
          msg: 'Can\'t get payment intent. Please contact to Perfect Forex support.'
        });
      }
    );
...
exports.createTransaction = async (req, res) => {
  ...
  const result = await client.createTransaction({
    ...
    success_url: '/',
    cancel_url: '/',
    ipn_url: 'https://www.perfectforextrade.com/price/coinpayments/webhook'
  });
